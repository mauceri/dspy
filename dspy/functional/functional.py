...
class TypedPredictor(dspy.Module):
    ...
    # Immonde patch de Christian Mauceri
    def extracte_json_valide(self,input_string):
        """
        Patch Christian Mauceri
        Cette fonction extrait un objet JSON valide au début d'une chaîne de caractères.
    
        :param input_string: La chaîne de caractères d'entrée
        :return: Un tuple contenant l'objet JSON extrait et la partie restante de la chaîne
        """
        json_obj = None
        end_index = -1
    
        # Cherche la position de fin de l'objet JSON valide
        ...
        # Si un objet JSON a été trouvé
        if json_obj is not None:
            return json_obj, input_string[end_index:]
        else:
            raise ValueError("Aucun objet JSON valide trouvé au début de la chaîne.")
            ...
            for completion in result.completions:
                ...
                for name, field in signature.output_fields.items():
                    try:
                        ...
                        objet_json_valide,reste = self.extracte_json_valide(value)
                        #print(f"****** {objet_json_valide}")
                        #print(f"****** {reste}")
                        value = json.dumps(objet_json_valide)
